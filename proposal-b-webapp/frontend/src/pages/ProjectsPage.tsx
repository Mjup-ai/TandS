...
interface ProjectDedupeCandidate {
  score: number;
  reasons: string[];
  left: ProjectOfferItem;
  right: ProjectOfferItem;
}

interface ProjectDedupeHistoryItem {
  id: string;
  createdAt: string;
  userId: string | null;
  payload: {
...
export default function ProjectsPage() {
  ...
  const [duplicates, setDuplicates] = useState<ProjectDedupeCandidate[]>([]);
  const [history, setHistory] = useState<ProjectDedupeHistoryItem[]>([]);
  ...
  const load = () => {
    ...
    Promise.all([
      apiFetch(`/api/project-offers?${params.toString()}`).then((r) => r.json()),
      apiFetch(`/api/project-dedupe-candidates?${params.toString()}`).then((r) => r.json()),
      apiFetch('/api/project-dedupe-history?limit=10').then((r) => r.json()),
    ])
  ...
  const mergeOffers = async (keepOfferId: string, mergeOfferId: string) => {
    if (!window.confirm('この2件を同一案件として束ねますか？')) return;
    const key = `${keepOfferId}:${mergeOfferId}`;
    setMergingKey(key);
    try {
      const res = await apiFetch('/api/project-dedupe-merge', {
        method: 'POST',
  ...
  const findMatches = async (projectOfferId: string, title: string) => {
    setMatchingId(projectOfferId);
    ...
    try {
      const res = await apiFetch('/api/match', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ projectOfferId }),
      });
    ...
    <div>
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">担当営業とキーワードで、自分が見るべき案件に絞れます。</p>
        </div>
      </div>
      ...
      <section className="mt-4 rounded-xl border border-amber-200 bg-amber-50/60 p-4 shadow-card">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <h3 className="text-sm font-semibold text-amber-900">重複候補</h3>
            <p className="mt-1 text-sm text-amber-800">複数営業マンに届いた同一案件っぽいものを、先にここで確認できます。</p>
          </div>
          <div className="text-sm font-medium text-amber-900">{duplicates.length} 件</div>
        </div>

        {duplicates.length === 0 ? (
          <p className="mt-3 text-sm text-amber-800/80">今の条件では重複候補は見つかっていません。</p>
        ) : (
          <div className="mt-4 space-y-3">
            {duplicates.map((candidate, index) => (
              <div key={`${candidate.left.id}-${candidate.right.id}-${index}`} className="rounded-xl border border-amber-200 bg-white p-4">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div className="text-sm font-semibold text-amber-900">候補スコア {candidate.score}</div>
                  <div className="flex flex-wrap gap-2">
                    {candidate.reasons.map((reason) => (
                      <span key={reason} className="rounded-md bg-amber-100 px-2 py-0.5 text-xs font-medium text-amber-800">
                        {reason}
                      </span>
                    ))}
                  </div>
                ...
                <div className="mt-3 grid gap-3 lg:grid-cols-2">
                  {[candidate.left, candidate.right].map((offer, offerIndex) => (
                    <div key={`${offer.id}-${offerIndex}`} className="rounded-lg border border-slate-200 bg-slate-50 p-3">
                      <div className="font-medium text-slate-800">{offer.project?.canonicalName || offer.rawEmail?.subject || '（件名なし）'}</div>
      ...
      <section className="mt-4 rounded-xl border border-slate-200 bg-white p-4 shadow-card">
        <div className="flex items-end justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">直近で同一案件として束ねた操作です。</p>
          </div>
              ...
              <div key={item.id} className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700">
                ...
                <div className="mt-2 grid gap-2 lg:grid-cols-2">
                  <div className="rounded-md bg-white p-2">
                    <div className="text-xs font-semibold text-slate-500">残した案件</div>
                    <div className="mt-1 font-medium text-slate-800">
                      {item.keepOffer?.project?.canonicalName || item.keepOffer?.rawEmail?.subject || item.payload?.keepOfferId || '—'}
                    </div>
                  ...
                  <div className="rounded-md bg-white p-2">
                    <div className="text-xs font-semibold text-slate-500">束ねた案件</div>
                    <div className="mt-1 font-medium text-slate-800">
                      {item.mergeOffer?.project?.canonicalName || item.mergeOffer?.rawEmail?.subject || item.payload?.mergeOfferId || '—'}
                    </div>
                    <div className="mt-1 text-xs text-slate-500">
        ...
        <section className="mt-4 rounded-xl border border-indigo-200 bg-indigo-50/60 p-4 shadow-card">
          <div className="flex items-end justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-indigo-800">{matchResults.length} 件の候補人材が見つかりました。</p>
            </div>
            <button type="button" className="btn-secondary text-sm" onClick={() => setMatchResults(null)}>閉じる</button>
          </div>
          {matchResults.length === 0 ? (
            <p className="mt-3 text-sm text-indigo-800/80">マッチする人材が見つかりませんでした。</p>
          ) : (
              ...
              {matchResults.map((mr, i) => (
                <li key={i} className={`rounded-lg border p-3 ${
                  ...
                  <div className="flex items-center justify-between gap-2">
                    <div>
                      <span className="font-medium text-slate-800">{mr.talentTitle}</span>
          ...
          <li key={o.id} className="card card-hover p-4">
            <div className="flex items-start justify-between gap-3">
              ...
              <button
                ...
                onClick={() => findMatches(o.id, o.project?.canonicalName || o.rawEmail?.subject || '（件名なし）')}
